...
const text = ref("");
const LeaveWordList = ref<any[]>([]);
onMounted(() => {
  if(useService.isServiceAvailable)  getLeaveWordListFunc();
});

async function getLeaveWordListFunc() {
  const res: any = await getLeaveWordList();
  // 过滤内容
...
const addLeaveWord = async () => {
  ...
  const res: any = await userLeaveWord(text.value);
  if (res.code === 200) {
...
<template>
  <div class="bg">
    ...
    <div>
      <div class="title">
        ...
        <el-button
          ...
          >留言</el-button
        >
        <span v-else style="color: grey; font-size: 0.9rem"
          >服务离线，留言功能不可用</span
        >
      </div>
      <el-divider />
      <div class="title_content">
        <span>欢迎访问留言板板块！</span>
        <span
          >留言板是一个方便与其他用户交流、分享观点和意见的平台。通过留言板，您可以：</span
        >
        <span
          >1、留下自己的留言：无论是对特定主题的讨论、对某篇文章的评论，还是向其他用户提问，您都可以在留言板上发表自己的留言。</span
        >
        <span
          >2、回复其他用户：看到其他用户的留言后，您可以进行回复和互动，分享自己的观点，提供帮助或者表达共鸣。</span
        >
        <span
          >3、进行在线交流：留言板不仅是一个留言的平台，还是一个与其他用户进行实时交流的地方。您可以通过留言板与他人进行私聊，分享更多的讨论和信息。</span
        >
        ...
        >
          <div>
            ...
            <div style="margin-top: 1rem; float: right">
              <el-button type="primary" @click="addLeaveWord" plain
                >提交</el-button
              >
            </div>
            <div
              style="margin-top: 1rem; margin-right: 1rem; float: right"
              @click="isShow = false"
            >
    ...
    <div v-if="useService.isServiceAvailable">
